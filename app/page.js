...
export default function Page() {
  ...
  const visibleRecords = useMemo(() => {
    ...
    return [...filtered].sort(
      (a, b) =>
        new Date(`${b.controlDate}T${b.controlTime}`) -
        new Date(`${a.controlDate}T${a.controlTime}`)
    );
  ...
  const roomSummary = useMemo(() => {
    ...
    const followUp = roomRecords.filter(
      (r) =>
        r.result === "Takip Gerektirir" ||
        r.result === "Yönetici İncelemesi Gerekli"
    ).length;
  ...
  const login = () => {
    ...
    const initialRoom = ROOM_DEFINITIONS[0]?.roomNo || "";
    setSelectedRoom(initialRoom);
    setForm(createEmptyForm(foundUser, initialRoom));
  };
    ...
    return (
      ...
      >
        ...
        >
          ...
          <p style={{ color: "#64748b", marginTop: 10, marginBottom: 24 }}>
            Rutin oda kontrolleri, öğrenci gözlemleri ve yönetici onayı
          </p>
      ...
      <div style={{ maxWidth: 1280, margin: "0 auto" }}>
        ...
        >
          ...
          <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
            <button
              onClick={() => setActiveView("new-control")}
              style={{
                padding: "10px 14px",
                borderRadius: 10,
                border: "1px solid #cbd5e1",
                background: activeView === "new-control" ? "#0f172a" : "white",
                color: activeView === "new-control" ? "white" : "#0f172a",
                cursor: "pointer",
              }}
            >
              Yeni Kontrol
            ...
            <button
              ...
              style={{
                ...
                borderRadius: 10,
                border: "1px solid #cbd5e1",
                background: activeView === "history" ? "#0f172a" : "white",
                color: activeView === "history" ? "white" : "#0f172a",
                cursor: "pointer",
              }}
            >
              Oda Geçmişi
            ...
            <button
              ...
              style={{
                ...
                borderRadius: 10,
                border: "1px solid #cbd5e1",
                background: activeView === "records" ? "#0f172a" : "white",
                color: activeView === "records" ? "white" : "#0f172a",
                cursor: "pointer",
              }}
            >
              Kayıtlar
              ...
              >
                {ROOM_DEFINITIONS.map((room) => (
                  <option key={room.roomNo} value={room.roomNo}>
                    {room.block} Blok / {room.floor}. Kat / Oda {room.roomNo}
                  </option>
                ))}
              </select>
            ...
            <div style={sectionCardStyle()}>
              ...
              {!previousControl ? (
                ...
                <div style={{ lineHeight: 1.9, fontSize: 14 }}>
                  ...
                  <div><strong>Kontrol Eden:</strong> {previousControl.controledBy || previousControl.controlledBy}</div>
                ...
                <div style={sectionCardStyle()}>
                  ...
                  <div style={{ marginTop: 14 }}>
                    ...
                    <textarea
                      ...
                      value={form.roomNote}
                      onChange={(e) => updateForm("roomNote", e.target.value)}
                      placeholder="Oda ile ilgili genel değerlendirme, düzensizlik, uyarı, tekrar eden sorun vb."
                    />
                  </div>
                  ...
                  <div style={{ marginTop: 14 }}>
                    ...
                    <textarea
                      ...
                      onChange={(e) => updateForm("securityNote", e.target.value)}
                      placeholder="Şüpheli unsur, dikkat gerektiren husus, yöneticiye not vb."
                    />
                  </div>
                </div>
                  ...
                  {form.students.map((student, index) => (
                    ...
                    >
                      <h3 style={{ marginTop: 0, marginBottom: 12 }}>
                        {student.name || `Öğrenci ${student.id}`}
                      </h3>
                      <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 12 }}>
                        {[
                          ...
                          <div key={key}>
                            ...
                            <select
                              style={fieldStyle()}
                              value={student[key]}
                              onChange={(e) => updateStudent(index, key, e.target.value)}
                            >
                              {options.map((item) => (
                  ...
                  roomHistory.map((record) => (
                    ...
                    >
                      ...
                      <div style={{ marginTop: 10 }}>
                        <button
                          onClick={() => loadRecordToDetail(record.id)}
                          style={{
                            padding: "10px 14px",
                            borderRadius: 10,
                            border: "1px solid #cbd5e1",
                    ...
                    >
                      <div style={{ display: "flex", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
                        <div>
                          ...
                          <div style={{ color: "#64748b", marginTop: 4 }}>
                            {record.createdByName} - {record.controlType} - {record.term}
                          </div>
                        </div>
                      ...
                      <div style={{ display: "flex", gap: 10, marginTop: 12, flexWrap: "wrap" }}>
                        ...
                        {user.role === "admin" && record.status !== "Onaylandı" ? (
                          <button
                            onClick={() => approveControl(record.id)}
                            style={{
                              padding: "10px 14px",
                              borderRadius: 10,
                              border: "none",
